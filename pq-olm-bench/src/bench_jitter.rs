...
/// Metric #10: Latency Jitter — Distribution, bimodality, coefficient of variation
///
/// Production-grade jitter analysis for 10Gb/s environments:
...
pub fn run(iterations: usize, warmup: usize) -> serde_json::Value {
    ...
    // ═══════════════════════════════════════════
    //  Classical: collect large sample set
    // ═══════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════
    //  PQ-OLM: collect large sample set
    // ═══════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════
    let cr = BenchResult::new("Classical Encrypt+Decrypt", "ns", classical_samples.clone());
    let pr = BenchResult::new("PQ Encrypt+Decrypt", "ns", pq_samples.clone());

    output::print_comparison_table("Latency Jitter (Encrypt + Decrypt, 256B)", &cr, &pr);
    ...
    println!("\n  Jitter Analysis:");
    println!("  ┌─────────────────────────┬────────────────┬────────────────┐");
    println!("  │ Metric                  │ Classical      │ PQ-OLM         │");
    println!("  ├─────────────────────────┼────────────────┼────────────────┤");
    ...
    // Stability assessment for 10Gb/s
    println!("\n  10Gb/s Stability Assessment:");
    let c_stable = c_jitter.cv < 0.1 && !c_jitter.is_bimodal;
    let p_stable = p_jitter.cv < 0.1 && !p_jitter.is_bimodal;
    println!("    Classical: {} (CV={:.4}, bimodal={})",
        if c_stable { "STABLE — suitable for predictable scheduling" }
        else { "VARIABLE — may need jitter buffers" },
        c_jitter.cv, c_jitter.is_bimodal);
    println!("    PQ-OLM:    {} (CV={:.4}, bimodal={})",
        if p_stable { "STABLE — suitable for predictable scheduling" }
        else { "VARIABLE — may need jitter buffers" },
    ...
    println!("\n  Classical Latency Distribution:");
    print_histogram(&c_hist);
    println!("\n  PQ-OLM Latency Distribution:");
...
fn analyze_jitter(samples: &[f64]) -> JitterAnalysis {
    ...
    // BC > 0.555 suggests bimodality (Sarle's rule)
...
fn histogram(samples: &[f64], buckets: usize) -> Vec<serde_json::Value> {
    ...
    let _max_count = *counts.iter().max().unwrap_or(&1);

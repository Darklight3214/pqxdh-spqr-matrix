...
pub fn run(iterations: usize, warmup: usize) -> serde_json::Value {
    ...
    // Print results
    let cr = BenchResult::new("X3DH Roundtrip", "ns", classical_roundtrip.clone());
    let pr = BenchResult::new("PQXDH Roundtrip", "ns", pq_roundtrip.clone());
    output::print_comparison_table("Handshake Roundtrip", &cr, &pr);

    let ci = BenchResult::new("X3DH Initiator", "ns", classical_initiator.clone());
    let pi = BenchResult::new("PQXDH Initiator", "ns", pq_initiator.clone());
    output::print_comparison_table("Handshake Initiator Only", &ci, &pi);

    let cre = BenchResult::new("X3DH Responder", "ns", classical_responder.clone());
    let pre = BenchResult::new("PQXDH Responder", "ns", pq_responder.clone());

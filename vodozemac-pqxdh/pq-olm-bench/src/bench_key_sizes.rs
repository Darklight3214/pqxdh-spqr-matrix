...
/// Metric #6: Key & Bundle Sizes — X3DH+DR vs PQXDH+SPQR
///
/// Compares all cryptographic artifact sizes and prekey bundle upload sizes,
/// showing the infrastructure cost of the PQ upgrade.
pub fn run() -> serde_json::Value {
    ...
    output::print_kv_table("Key & Artifact Sizes: X3DH+DR vs PQXDH+SPQR", &rows);
    ...
    println!("    X3DH+DR    : {} B", classical_bundle);
    println!("    PQXDH+SPQR : {} B  (+{} B, {:.1}x)",
        pq_bundle,
    ...
    serde_json::json!({
        ...
        "bundles": {
            "x3dh_dr_bundle_bytes": classical_bundle,
            "pqxdh_spqr_bundle_bytes": pq_bundle,

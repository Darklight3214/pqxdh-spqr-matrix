...
use super::{
    ...
    shared_secret::{RemoteShared3DHSecret, Shared3DHSecret, SharedPQXDHSecret},
};
use crate::{
...
pub struct Account {
    ...
    fallback_keys: FallbackKeys,
    /// The semi-permanent Curve25519 signed prekey for PQXDH.
    /// Signed by the Ed25519 identity key to prove ownership.
    signed_prekey: Option<Curve25519Keypair>,
}
...
impl Account {
    ...
    /// Generate a new signed prekey for PQXDH.
    /// The prekey is a Curve25519 key, signed by our Ed25519 identity key.
    /// Returns (public_key, signature).
    pub fn generate_signed_prekey(&mut self) -> (Curve25519PublicKey, Ed25519Signature) {
        let keypair = Curve25519Keypair::new();
        let public_key = keypair.public_key();
        let signature = self.signing_key.sign(public_key.as_bytes());
        self.signed_prekey = Some(keypair);
        (public_key, signature)
    }

    /// Get the public part of the current signed prekey.
    pub fn signed_prekey_public(&self) -> Option<Curve25519PublicKey> {
        self.signed_prekey.as_ref().map(|k| k.public_key())
    }
    ...
    /// Creates a new Olm [`Session`] using PQXDH key agreement (post-quantum secure).
    ///
    /// This combines 4 X25519 DH operations with ML-KEM-768 encapsulation.
    ///
    /// # Returns
    /// A tuple of `(Session, kem_ciphertext)` - the ciphertext must be sent to the recipient.
    pub fn create_outbound_session_pqxdh(
        &self,
        session_config: SessionConfig,
        ...
        let rng = thread_rng();
        let ephemeral_key = ReusableSecret::random_from_rng(rng);
        let public_ephemeral_key = Curve25519PublicKey::from(&ephemeral_key);

        // ML-KEM-768 encapsulation
        let kem = Kem::new(Algorithm::MlKem768).expect("ML-KEM-768 not available");
        let kem_pk = kem.public_key_from_bytes(kem_public_key)
            .expect("Invalid KEM public key");
        let (kem_ciphertext, kem_shared_secret) = kem.encapsulate(&kem_pk)
            .expect("KEM encapsulation failed");


        // Extract KEM shared secret bytes before passing to constructor
        let kem_ss_bytes = kem_shared_secret.into_vec();

        // Build PQXDH shared secret (4 DH + KEM)
        ...
        let session_keys = SessionKeys {
            identity_key: self.curve25519_key(),
            base_key: public_ephemeral_key,
            one_time_key: remote_one_time_key.unwrap_or(remote_signed_prekey),
        };

        let session = Session::new_pqxdh(session_config, shared_secret, session_keys);

        (session, kem_ciphertext.into_vec())
    }
    ...
    /// Creates an inbound Olm [`Session`] using PQXDH (post-quantum secure).
    ///
    /// Bob uses this to establish a session from Alice's PreKeyMessage.
    pub fn create_inbound_session_pqxdh(
        &mut self,
        their_identity_key: Curve25519PublicKey,
        pre_key_message: &PreKeyMessage,
        kem_ciphertext: &[u8],
        ...
        use oqs::kem::{Kem, Algorithm};
        use super::shared_secret::RemoteSharedPQXDHSecret;

        if their_identity_key != pre_key_message.identity_key() {
            return Err(SessionCreationError::MismatchedIdentityKey(
                their_identity_key,
                pre_key_message.identity_key(),
            ));
        }

        // Find the matching private OTK
        let public_otk = pre_key_message.one_time_key();
        let private_otk = self
            .find_one_time_key(&public_otk)
            .ok_or(SessionCreationError::MissingOneTimeKey(public_otk))?;
        ...
        // KEM decapsulation
        let kem = Kem::new(Algorithm::MlKem768).expect("ML-KEM-768 not available");
        let kem_sk = kem.secret_key_from_bytes(kem_secret_key)
            .expect("Invalid KEM secret key");
        let kem_ct = kem.ciphertext_from_bytes(kem_ciphertext)
            .expect("Invalid KEM ciphertext");
        let kem_shared_secret = kem.decapsulate(&kem_sk, &kem_ct)
            .expect("KEM decapsulation failed");

        // Build PQXDH shared secret (responder side)
        let kem_ss_bytes = kem_shared_secret.into_vec();
        let shared_secret = RemoteSharedPQXDHSecret::new(
            self.diffie_hellman_key.secret_key(),
            self.signed_prekey.as_ref().expect("No signed prekey generated").secret_key(),
            Some(private_otk),
            &pre_key_message.identity_key(),
            &pre_key_message.base_key(),
            Some(&kem_ss_bytes),
        ...
        // Create Session with PQXDH
        let mut session = Session::new_remote_pqxdh(
            config,
            shared_secret,
            pre_key_message.message.ratchet_key,
            session_keys,
        ...
        // Decrypt to verify session is valid
        let plaintext = session.decrypt_decoded(&pre_key_message.message)?;
...
impl From<AccountPickle> for Account {
    fn from(pickle: AccountPickle) -> Self {
        Self {
            ...
            signed_prekey: pickle.signed_prekey,
        }
    }
}
